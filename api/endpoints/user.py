...
from core.GerericResponse import GenericResponse
...
@router.get("",
            ...
            response_model=GenericResponse[UserListItem]
            )
async def user_list(
        pageSize: int = 10,
        current: int = 1,
    ...
    #logger.info(f"i am app_log log something: {user_data}")
    # 总数
    total = await user_data.count()
    # 查询
    data = await user_data.limit(pageSize).offset(pageSize * (current - 1)).order_by("-create_time").all()
    user_items = [UserListItem(**item.__dict__) for item in data]
    return GenericResponse.res_antd(data=user_items, total=total)
